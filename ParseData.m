...
-(NSArray *) uniqueCourseArray
{
	...
	int rawCourseCount = rawArray.count;

	for (x=0; x<rawCourseCount; x++)
	{
		...
		NSNumber * courseHashObject = [NSNumber numberWithInt:courseHash];
...
{
	roundCount = [NSNumber numberWithInt: [roundCount integerValue]+1];
	...
	[query getFirstObjectInBackgroundWithBlock:^(PFObject* user, NSError * error)
	 {
		 if(!error)
		 {
			int fullVersion = [[user valueForKey:@"HandicapCalculatorFullVersion"]integerValue];
